...
- 백엔드는 client_secret 사용 중이라고 함

백엔드 확인 대기 (로그의 KOE 코드 한 줄이면 확정):

1. **교환 시 redirect_uri** — 요청 본문의 `redirectUri`를 그대로 쓰는지, 고정값인지. 인가 때 주소와 다르면 `KOE320`. 판별 팁: 배포 프론트에서는 되고 로컬만 실패하면 이 케이스
2. **client_secret 값** — 재발급/오타면 `invalid_client`
